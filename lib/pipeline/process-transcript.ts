import { z } from "zod"
import { extractAddressDetails, formatAddressForQuote, type AddressExtractionResult } from "../address-extraction"
...
import { isTruePlantCatalogItem } from "../plant-item-classification"
...
import { normalizeRetainingProcessedQuote, correctMisclassifiedRetaining } from "../retaining-processing"
import { isPrimaryTrade, type PrimaryTrade } from "../trade-profile"
...
import { type QuoteSpecialist, getSpecialistInstructions, getSharedUniversalExtractionInstructions } from "../quote-specialist-instructions"
import {
...
export type ProcessTranscriptInput = {
  transcript: string
  /** Raw request-body template context (normalised internally). */
  templateContext?: unknown
  /** Raw request-body knowledge-item context (normalised internally). */
  knowledgeItemContext?: unknown
  /** Raw request-body primary trade (normalised internally). */
  primaryTrade?: unknown
  inputMethod?: "paste" | "voice" | "test"
  userId?: string
}

export type ProcessTranscriptDeps = {
  /** OpenAI-backed classification. Injectable so tests can run headlessly. */
...
/**
 * Shared production pipeline: raw transcript → fully processed ProcessedQuote
 * (deterministic post-processing + calculators + labour recovery + Quote
 * Auditor). The API route (app/api/process-quote/route.ts) is a thin wrapper
 * over this. Tests call it directly with mocked `classify` / `extractQuote`
 * deps, so no OpenAI call and no browser are required. Runtime behaviour is
 * identical to the previous in-route implementation.
 */
export async function processTranscriptToQuote(
  ...
  const knowledgeItemContext = getKnowledgeItemContext(input.knowledgeItemContext)
  const primaryTrade = getPrimaryTrade(input.primaryTrade)

  const leadDetails = extractLeadDetails(transcript)
  log.log("process-quote lead details", {
    ...
    try {
      classification = await classify(transcript, primaryTrade)
    } catch (error) {
      log.log("process-quote classification fallback", {
    ...
    try {
      ...
      const quote = normalizeFencingProcessedQuote(correctMisclassifiedRetaining(normalizeRetainingProcessedQuote(applyAddressReviewDetails(
        ...
        leadDetails,
      ), transcript), transcript), transcript) as ProcessedQuote
      ...
      // Cast preserves the previous route behaviour (this arg was passed as-is; the
      // route only compiled because next.config sets typescript.ignoreBuildErrors).
      const resolvableKnowledgeItems = knowledgeItemContext as unknown as ResolvableItem[]
      applyDeckingBillOptions(quote, transcript, resolvableKnowledgeItems)
      applyRetainingBillOptions(quote, transcript, resolvableKnowledgeItems)
      applyPavingBillOptions(quote, transcript, resolvableKnowledgeItems)
      ...
      return { quote, fallbackUsed: false, classification, leadDetails }
    } catch (error) {
      const message = getErrorMessage(error)
      log.log("process-quote fallback used", {
        client_name: leadDetails.client_name,
        site_address: leadDetails.site_address,
        confidence: leadDetails.confidence,
        model: QUOTE_MODEL,
      ...
      log.log("process-quote extraction reliability", {
        model: QUOTE_MODEL,
        transcript_length: transcript.length,
        reliability_metric: "failed",
      ...
      const fallback = normalizeFencingProcessedQuote(correctMisclassifiedRetaining(normalizeRetainingProcessedQuote(fallbackQuote(transcript, leadDetails, classification, message, templateContext), transcript), transcript), transcript) as ProcessedQuote
      return { quote: fallback, fallbackUsed: true, classification, leadDetails }
    }
}
