...
export function scoreTemplatesForQuote({
  ...
  return templates
    .map((template) => {
      ...
      const templateNameKeywords = keywordSet([template.name, template.template_name, template.trade, template.job_type].filter(Boolean).join(" "))
      const keywordMatches = [...quoteKeywords].filter((keyword) => templateKeywords.has(keyword))
      ...
      const score = metadataScore + categoryMatches.length * 2 + keywordMatches.length + nameKeywordMatches.length * 4
